...
def stats_event_college(request, e_id, c_id):
    ...
    try:
        ...
        messages.warning(request,'College rep not selected. Please select college rep first.')
        return redirect(reverse('pcradmin:stats_event',kwargs={'e_id':event.id}))
...
@staff_member_required
def final_email_send_2(request, c_id):
    # email_group = EmailGroup.objects.get(id = eg_id)
    # participants = email_group.participant_set.all()
    college = College.objects.get(id = c_id)
    participants = Participant.objects.filter(pcr_final=True, college=college)
    emailgroup = EmailGroup.objects.create()
    eg_id = emailgroup.id
    for participant in participants:
...
@staff_member_required
def final_email_resend(request):
    c = College.objects.all()
    print(c)
    return render(request, 'pcradmin/final_email_resend.html',{'colleges':c})